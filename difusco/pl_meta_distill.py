...
class COMetaModel(pl.LightningModule):
  def __init__(self,
    ...
    if self.diffusion_type == 'gaussian':
      ...
      self.diffusion = GaussianDiffusion(
          T=self.diffusion_steps, schedule=self.diffusion_schedule, skip=self.skip)
  ...
  def categorical_posterior(self, target_t, t, x0_pred_prob, xt):
    ...
    if self.sparse:
      xt = xt.reshape(-1)
    return xt
  def batched_gaussian_posterior(self, target_t, t, pred, xt):
    """same as gaussian_posterior but for batched inputs
    """
    diffusion = self.diffusion
    if target_t is None:
      target_t = t - 1
    ...
    atbar = torch.from_numpy(atbar).to(device=pred.device)
    atbar_target = torch.from_numpy(atbar_target).to(device=pred.device)

    if self.args.inference_trick is None or t.any() <= 1:
      # Use DDPM posterior
      at = diffusion.alpha[t]
      at = torch.from_numpy(at).to(device=pred.device)
      z = torch.randn_like(xt)
      atbar_prev = diffusion.alphabar[t - 1]
      atbar_prev = torch.from_numpy(atbar_prev).to(device=pred.device)
      beta_prev = diffusion.beta[t - 1]
      beta_prev = torch.from_numpy(beta_prev).to(device=pred.device)
      beta_tilde = beta_prev * (1 - atbar_prev) / (1 - atbar)

      xt_target = (1 / torch.sqrt(at)).unsqueeze(1).unsqueeze(2) * (xt - ((1 - at) / torch.sqrt(1 - atbar)).unsqueeze(1).unsqueeze(2) * pred)
      xt_target = xt_target + torch.sqrt(beta_tilde).unsqueeze(1).unsqueeze(2) * z
    elif self.args.inference_trick == 'ddim':
      #batched xt_target 
      xt_target = torch.sqrt(atbar_target / atbar).unsqueeze(1).unsqueeze(2) * (xt - torch.sqrt(1 - atbar).unsqueeze(1).unsqueeze(2)  * pred)
      xt_target = xt_target + torch.sqrt(1 - atbar_target).unsqueeze(1).unsqueeze(2) * pred
    else:
      raise ValueError('Unknown inference trick {}'.format(self.args.inference_trick))
    
    #strange broadcasting error above, had to use unsqueeze to manually add dimensions
    return xt_target
...
class COMetaModel_teacher(torch.nn.Module):
  def __init__(self,
               param_args,
               node_feature_only=False):
    super(COMetaModel_teacher, self).__init__()
    self.args = param_args
    self.diffusion_type = self.args.diffusion_type
    self.diffusion_schedule = self.args.diffusion_schedule
    self.diffusion_steps = self.args.diffusion_steps
